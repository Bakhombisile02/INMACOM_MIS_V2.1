...
interface FlowLevelsProps {
    stations: StationFlowRow[];
    pendingQueue: PendingFlowRow[];
    historicalLogs: HistoricalFlowRow[];
    canManage: boolean;
    userRole: string;
}

export default function FlowLevels({
    stations,
    pendingQueue,
    historicalLogs,
    canManage,
    ...
    const mappedStations: GisStationData[] = stations.map((station) => {
        ...
        if (station.value !== null) {
            if (station.limit !== null) {
                if (station.value >= station.limit) {
                    color = 'rgb(0, 51, 204)'; // Compliant (Blue)
                ...
                color = 'rgb(146, 208, 80)'; // Default green if no limit exists
                statusText = 'Normal';
            }
        }
        ...
        return {
            ...
            color,
            popupData: [
                { label: t('flow.popup.flowRate'), value: station.value !== null ? `${station.value} ${station.unit}` : t('common.noData'), color },
                { label: t('flow.popup.iimaLimit'), value: station.limit !== null ? `${station.limit} ${station.unit}` : 'N/A' },
                { label: t('flow.popup.status'), value: statusText, color },
                { label: t('flow.popup.dateTime'), value: station.date ? new Date(station.date).toLocaleString() : '—' },
            ],
        };
    });

    const criticalStations = stations.filter(s => s.value !== null && s.limit !== null && s.value < s.limit);

    const legends = [
        { color: 'rgb(0, 51, 204)', label: t('flow.popup.compliant') },
        { color: 'rgb(255, 0, 0)', label: t('flow.popup.nonCompliant') },
        { color: 'rgb(146, 208, 80)', label: t('flow.popup.noLimit') },
        { color: 'rgb(127, 127, 127)', label: t('flow.popup.noValue') },
    ];
    ...
    const handleCreateSubmit = (e: React.FormEvent) => {
        ...
        createForm.post(route('measurements.store'), {
            onSuccess: () => {
                ...
                createForm.reset();
                notifications.show({
                    title: t('flow.notify.logged'),
                    message: t('flow.notify.loggedMsg'),
                    color: 'green',
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        ...
        editForm.patch(route('measurements.update', selectedMeasurement.id), {
            onSuccess: () => {
                closeEdit();
                notifications.show({
                    title: t('flow.notify.updated'),
                    message: t('flow.notify.updatedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    const handleDelete = (id: string) => {
        if (!confirm(t('flow.notify.deleteConfirm'))) return;

        router.delete(route('measurements.destroy', id), {
            onSuccess: () => {
                notifications.show({
                    title: t('flow.notify.deleted'),
                    message: t('flow.notify.deletedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    const handleApprove = (id: string) => {
        router.post(route('measurements.approve', id), {}, {
            onSuccess: () => {
                notifications.show({
                    title: t('flow.notify.approved'),
                    message: t('flow.notify.approvedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    const handleRejectSubmit = (e: React.FormEvent) => {
        ...
        rejectForm.post(route('measurements.reject', selectedMeasurement.id), {
            onSuccess: () => {
                closeReject();
                notifications.show({
                    title: t('flow.notify.rejected'),
                    message: t('flow.notify.rejectedMsg'),
                    color: 'yellow',
                    icon: <IconX size={18} />,
                    autoClose: 5000,
                });
    ...
    return (
        <>
            <Head title={tNav('flowLevels')} />

            <Container size="xl" py="xl">
                {/* Header Section */}
                <Flex justify="space-between" align="center" mb="xl">
                    <Stack gap="xs">
                        <Title order={1}>{tNav('flowLevels')}</Title>
                        <Text c="dimmed">{t('flow.description')}</Text>
                    </Stack>
                    <Group gap="md">
                        {selectedStationId && (
                            <Button
                        ...
                        <a href={route('flow-levels.export-csv')}>
                            <Button
                                variant="default"
                    ...
                    <Grid gutter="lg">
                        ...
                        <Grid.Col span={{ base: 12, lg: 5 }}>
                            <Stack gap="lg">
                                ...
                                <SimpleGrid cols={2} spacing="md">
                                    <Card withBorder radius="lg" p="md" style={{ overflow: 'hidden' }}>
                                        <Group justify="space-between" align="flex-start" wrap="nowrap">
                                            <Stack gap="xs">
                                                <Text size="xs" c="dimmed" fw={700} style={{ textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                                    {t('flow.stats.basinAvgFlow')}
                                                </Text>
                                                <Text fw={800} style={{ fontSize: '1.6rem', lineHeight: 1 }}>
                                                    {(stations.filter(s => s.value !== null).reduce((sum, s) => sum + (s.value ?? 0), 0) / Math.max(1, stations.filter(s => s.value !== null).length)).toFixed(2)} <span style={{ fontSize: '1rem', fontWeight: 500 }}>m³/s</span>
                                                </Text>
                                            </Stack>
                                        ...
                                        <Group gap="xs" mt="xs" align="center">
                                            ...
                                            <Text size="11px" c="dimmed">
                                                <Text span c="teal.7" fw={700}>{t('common.active')}</Text> {t('flow.stats.acrossSubbasins')}
                                            </Text>
                                        </Group>
                                    </Card>
                                    ...
                                    <Card withBorder radius="lg" p="md" style={{ overflow: 'hidden', cursor: criticalStations.length > 0 ? 'pointer' : 'default' }} onClick={criticalStations.length > 0 ? openCritical : undefined}>
                                        <Group justify="space-between" align="flex-start" wrap="nowrap">
                                            <Stack gap="xs">
                                                <Text size="xs" c="dimmed" fw={700} style={{ textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                                    {t('flow.stats.alertGauges')}
                                                </Text>
                                                <Text fw={800} style={{ fontSize: '1.6rem', lineHeight: 1 }} c={stations.filter(s => s.value !== null && s.limit !== null && s.value < s.limit).length > 0 ? 'red.7' : 'dark'}>
                                                    {stations.filter(s => s.value !== null && s.limit !== null && s.value < s.limit).length} <span style={{ fontSize: '1rem', fontWeight: 500 }}>/ {stations.filter(s => s.value !== null).length}</span>
                                                </Text>
                                            </Stack>
                                            <ThemeIcon variant="light" size="xl" radius="md" color={stations.filter(s => s.value !== null && s.limit !== null && s.value < s.limit).length > 0 ? 'red' : 'gray'}>
                                                <IconAlertTriangle size={22} />
                                            </ThemeIcon>
                                        </Group>
                                        <Group gap="xs" mt="xs" align="center">
                                            {stations.filter(s => s.value !== null && s.limit !== null && s.value < s.limit).length > 0 ? (
                                                <>
                                                    <IconAlertTriangle size={14} color="var(--mantine-color-red-7)" />
                                                    <Text size="11px" c="dimmed">
                                                        <Text span c="red.7" fw={700}>{t('flow.stats.exceedanceDetected')}</Text> {t('flow.stats.complianceMargins')}
                                                    </Text>
                                                </>
                                            ) : (
                                                <>
                                                    <IconCheck size={14} color="var(--mantine-color-teal-7)" />
                                                    <Text size="11px" c="dimmed">
                                                        <Text span c="teal.7" fw={700}>{t('flow.stats.normal')}</Text> {t('flow.stats.complianceMargins')}
                                                    </Text>
                                                </>
                                            )}
                                        </Group>
                                ...
                                <Card withBorder radius="md" p="md">
                                    <Title order={4} mb="xs">{t('flow.registry.title')}</Title>
                                    <ScrollArea h={450}>
                                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                                <Table.Tr>
                                                    ...
                                                    <Table.Th style={{ fontSize: 11 }}>{t('flow.registry.flowRate')}</Table.Th>
                                                </Table.Tr>
                                            </Table.Thead>
                                            <Table.Tbody>
                        ...
                        <Grid.Col span={{ base: 12, lg: 7 }}>
                            <GisMap
                                ...
                                legendTitle={t('flow.legend.compliance')}
                                legends={legends}
                                height={560}
                            ...
                            <Card withBorder radius="md" p="lg">
                                <Flex justify="space-between" align="flex-start" wrap="wrap" gap="md">
                                    ...
                                    <Stack align="flex-end" gap={2}>
                                        <Text size="xs" fw={700} c="dimmed">{t('flow.iima.limit')}</Text>
                                        <Title order={2} c="red.7">{targetStation.limit !== null ? `${targetStation.limit} m³/s` : 'N/A'}</Title>
                                    </Stack>
                                </Flex>
                            </Card>
                        )}
                        ...
                        <Grid gutter="lg">
                            ...
                            <Grid.Col span={{ base: 12, lg: 8 }}>
                                <SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
                                    <Card withBorder radius="md" p="md">
                                        <Title order={4} mb="md">{t('flow.chart.fdc')}</Title>
                                        {historicalLoading ? (
                                        ...
                                        <Text size="10px" c="dimmed" mt="xs" ta="center">
                                            {t('flow.chart.fdcDesc')}
                                        </Text>
                                    </Card>

                                    <Card withBorder radius="md" p="md">
                                        <Title order={4} mb="md">{t('flow.chart.historical')}</Title>
                                        {historicalLoading ? (
                                            <Flex justify="center" align="center" h={240}><IconRefresh className="animate-spin" /></Flex>
                                        ) : (
                            ...
                            <Grid.Col span={{ base: 12, lg: 4 }}>
                                <Card withBorder radius="md" p="0" style={{ overflow: 'hidden', height: 320 }}>
                                    ...
                                    legendTitle={t('flow.legend.stationCenter')}
                                        legends={legends}
                                        height={320}
                ...
                <Card withBorder radius="md" p="md" mt="xl">
                    ...
                    <ScrollArea h={400}>
                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                <Table.Tr>
                                    <Table.Th style={{ fontSize: 11 }}>{t('common.table.gauge')}</Table.Th>
                                    <Table.Th style={{ fontSize: 11 }}>{t('flow.history.dischargeHeader')}</Table.Th>
                                ...
                                {historicalLogs.length > 0 ? (
                                    ...
                                    <Table.Tr>
                                        <Table.Td colSpan={8} style={{ textAlign: 'center', padding: 16 }}>
                                            <Text size="xs" c="dimmed">{t('flow.empty.measurements')}</Text>
                                        </Table.Td>
                                    </Table.Tr>
                                )}
                            </Table.Tbody>
                ...
                {canManage && pendingQueue.length > 0 && (
                    <Card withBorder radius="md" p="md" mt="xl" style={{ borderLeft: '5px solid #d97706' }}>
                        <Title order={4} mb="md" c="amber.8">{t('flow.queue.title')} ({pendingQueue.length})</Title>
                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                <Table.Tr>
                                    <Table.Th style={{ fontSize: 11 }}>{t('common.table.gauge')}</Table.Th>
                                    <Table.Th style={{ fontSize: 11 }}>{t('flow.queue.dischargeHeader')}</Table.Th>
                                    <Table.Th style={{ fontSize: 11 }}>{t('common.table.readingDate')}</Table.Th>
            ...
            {/* Modal: Log Discharge */}
            <Modal opened={createOpened} onClose={closeCreate} title={<Text fw={700} size="md">{t('flow.modal.log')}</Text>} centered radius="md">
                <form onSubmit={handleCreateSubmit}>
                    <Stack gap="md">
                        <Select
                            label={t('flow.modal.selectStation')}
                            placeholder={t('flow.modal.stationPlaceholder')}
                        ...
                        <NumberInput
                            label={t('flow.modal.valueLabel')}
                            placeholder={t('flow.modal.valuePlaceholder')}
            ...
            {/* Modal: Edit Discharge */}
            <Modal opened={editOpened} onClose={closeEdit} title={<Text fw={700} size="md">{t('flow.modal.edit')}</Text>} centered radius="md">
                {selectedMeasurement && (
            ...
            <Modal opened={rejectOpened} onClose={closeReject} title={<Text fw={700} size="md" c="red.7">{t('flow.modal.reject')}</Text>} centered radius="md">
                {selectedMeasurement && (
            ...
            <Modal
                ...
                title={
                    <Group gap="xs">
                        ...
                        <Title order={5}>{t('flow.stats.alertGauges')} — {criticalStations.length}</Title>
                    </Group>
                }
                zIndex={2000}
            >
                <Table highlightOnHover verticalSpacing="xs">
                    <Table.Thead>
                        <Table.Tr>
                            ...
                            <Table.Th style={{ fontSize: 11 }}>Flow (m³/s)</Table.Th>
                            <Table.Th style={{ fontSize: 11 }}>IIMA Limit</Table.Th>
                        </Table.Tr>
                    </Table.Thead>
                    <Table.Tbody>
                        {criticalStations.map((s) => (
                            ...
                            >
                                ...
                                <Table.Td><Text fz="xs" c="dimmed">{s.limit != null ? `${s.limit} ${s.unit}` : 'N/A'}</Text></Table.Td>
                            </Table.Tr>
                        ))}
                    </Table.Tbody>
                </Table>
            ...
            <MeasurementImportModal type="flow" opened={importOpen} onClose={closeImport} />
        </>
    );
}

FlowLevels.layout = (page: React.ReactNode) => <AuthenticatedLayout>{page}</AuthenticatedLayout>;
