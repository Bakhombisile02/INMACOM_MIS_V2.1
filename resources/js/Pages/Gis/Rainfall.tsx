...
import { RainfallHydrographChart } from '@/Components/Gis/HistoricalCharts';
import {
...
interface RainfallProps {
    stations: StationRainRow[];
    pendingQueue: PendingRainRow[];
    historicalLogs: HistoricalRainRow[];
    canManage: boolean;
    userRole: string;
}
...
export default function Rainfall({
    ...
    const mappedStations: GisStationData[] = stations.map((station) => {
        ...
        if (val !== null) {
            if (val >= 50) {
                color = 'rgb(0, 51, 204)'; // Sufficient (Blue)
                statusText = 'Sufficient (>= 50 mm)';
            } else {
                color = 'rgb(255, 0, 0)'; // Below Normal (Red)
                statusText = 'Below Normal (< 50 mm)';
            }
        }
        ...
        return {
            ...
            color,
            popupData: [
                { label: t('rainfall.popup.cumulativeRain'), value: station.value !== null ? `${station.value} ${station.unit}` : t('common.noData'), color },
                { label: t('rainfall.popup.status'), value: statusText, color },
                { label: t('rainfall.popup.operatingOrg'), value: station.owner_org ?? '—' },
                { label: t('rainfall.popup.lastReading'), value: station.date ? new Date(station.date).toLocaleString() : '—' },
            ],
        };
    });

    const criticalStations = stations.filter(s => s.value !== null && s.value < 50);

    const legends = [
        { color: 'rgb(0, 51, 204)', label: t('rainfall.legend.sufficient') },
        { color: 'rgb(255, 0, 0)', label: t('rainfall.legend.belowNormal') },
        { color: 'rgb(127, 127, 127)', label: t('rainfall.legend.noValue') },
        { color: 'rgb(5,196,188)', label: t('rainfall.legend.incoatiBound') },
        { color: 'rgb(210,109,84)', label: t('rainfall.legend.maputoBound') },
    ];
    ...
    const handleDeepDive = async (stationId: string) => {
        ...
        try {
            const res = await fetch(`/stations/${stationId}/historical-data?type=rainfall`);
    ...
    const handleCreateSubmit = (e: React.FormEvent) => {
        ...
        createForm.post(route('measurements.store'), {
            onSuccess: () => {
                ...
                createForm.reset();
                notifications.show({
                    title: t('rainfall.notify.logged'),
                    message: t('rainfall.notify.loggedMsg'),
                    color: 'green',
            ...
            onError: (err) => {
                notifications.show({
                    title: t('rainfall.notify.logFailed'),
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        ...
        editForm.patch(route('measurements.update', selectedMeasurement.id), {
            onSuccess: () => {
                closeEdit();
                notifications.show({
                    title: t('rainfall.notify.updated'),
                    message: t('rainfall.notify.updatedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    // Delete rainfall reading
    const handleDelete = (id: string) => {
        if (!confirm(t('rainfall.notify.deleteConfirm'))) return;

        router.delete(route('measurements.destroy', id), {
            onSuccess: () => {
                notifications.show({
                    title: t('rainfall.notify.deleted'),
                    message: t('rainfall.notify.deletedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    const handleApprove = (id: string) => {
        router.post(route('measurements.approve', id), {}, {
            onSuccess: () => {
                notifications.show({
                    title: t('rainfall.notify.approved'),
                    message: t('rainfall.notify.approvedMsg'),
                    color: 'green',
                    icon: <IconCheck size={18} />,
                    autoClose: 5000,
                });
    ...
    const handleRejectSubmit = (e: React.FormEvent) => {
        ...
        rejectForm.post(route('measurements.reject', selectedMeasurement.id), {
            onSuccess: () => {
                closeReject();
                notifications.show({
                    title: t('rainfall.notify.rejected'),
                    message: t('rainfall.notify.rejectedMsg'),
                    color: 'yellow',
                    icon: <IconX size={18} />,
                    autoClose: 5000,
                });
    ...
    const targetHistorical = useMemo(
        () => (historicalData?.readings ?? []).filter((r: any) => r.measurement_type === 'rainfall'),
        [historicalData],
    );
    ...
    const historyRows = useMemo(() => {
        ...
        const stationRows = (historicalData?.history_logs as HistoricalRainRow[] | undefined)
    ...
    return (
        <>
            <Head title={tNav('rainfall')} />

            <Container size="xl" py="xl">
                {/* Header Section */}
                <Flex justify="space-between" align="center" mb="xl">
                    <Stack gap="xs">
                        <Title order={1}>{tNav('rainfall')}</Title>
                        <Text c="dimmed">{t('rainfall.description')}</Text>
                    </Stack>
                    <Group gap="md">
                        {selectedStationId && (
                            <Button
                        ...
                        <a href={route('rainfall.export-csv')}>
                            <Button
                                variant="default"
                    ...
                    <Grid gutter="lg">
                        ...
                        <Grid.Col span={{ base: 12, lg: 5 }}>
                            <Stack gap="lg">
                                ...
                                <SimpleGrid cols={2} spacing="md">
                                    <Card withBorder radius="lg" p="md" style={{ overflow: 'hidden' }}>
                                        <Group justify="space-between" align="flex-start" wrap="nowrap">
                                            <Stack gap="xs">
                                                <Text size="xs" c="dimmed" fw={700} style={{ textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                                    {t('rainfall.stats.basinAvgRain')}
                                                </Text>
                                                <Text fw={800} style={{ fontSize: '1.6rem', lineHeight: 1 }}>
                                                    {(stations.filter(s => s.value !== null).reduce((sum, s) => sum + (s.value ?? 0), 0) / Math.max(1, stations.filter(s => s.value !== null).length)).toFixed(1)} <span style={{ fontSize: '1rem', fontWeight: 500 }}>mm</span>
                                                </Text>
                                            </Stack>
                                        ...
                                        <Group gap="xs" mt="xs" align="center">
                                            ...
                                            <Text size="11px" c="dimmed">
                                                <Text span c="teal.7" fw={700}>{t('common.active')}</Text> {t('rainfall.stats.meteorologicalNetwork')}
                                            </Text>
                                        </Group>
                                    </Card>
                                    ...
                                    <Card withBorder radius="lg" p="md" style={{ overflow: 'hidden', cursor: criticalStations.length > 0 ? 'pointer' : 'default' }} onClick={criticalStations.length > 0 ? openCritical : undefined}>
                                        <Group justify="space-between" align="flex-start" wrap="nowrap">
                                            <Stack gap="xs">
                                                <Text size="xs" c="dimmed" fw={700} style={{ textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                                    {t('rainfall.stats.dryAlertStations')}
                                                </Text>
                                                <Text fw={800} style={{ fontSize: '1.6rem', lineHeight: 1 }} c={stations.filter(s => s.value !== null && s.value < 50).length > 0 ? 'red.7' : 'dark'}>
                                                    {stations.filter(s => s.value !== null && s.value < 50).length} <span style={{ fontSize: '1rem', fontWeight: 500 }}>/ {stations.filter(s => s.value !== null).length}</span>
                                                </Text>
                                            </Stack>
                                            <ThemeIcon variant="light" size="xl" radius="md" color={stations.filter(s => s.value !== null && s.value < 50).length > 0 ? 'red' : 'gray'}>
                                                <IconAlertTriangle size={22} />
                                            </ThemeIcon>
                                        </Group>
                                        <Group gap="xs" mt="xs" align="center">
                                            {stations.filter(s => s.value !== null && s.value < 50).length > 0 ? (
                                                <>
                                                    <IconAlertTriangle size={14} color="var(--mantine-color-red-7)" />
                                                    <Text size="11px" c="dimmed">
                                                        <Text span c="red.7" fw={700}>{t('rainfall.stats.lowPrecipitation')}</Text> {t('rainfall.stats.basinLevels')}
                                                    </Text>
                                                </>
                                            ) : (
                                                <>
                                                    <IconCheck size={14} color="var(--mantine-color-teal-7)" />
                                                    <Text size="11px" c="dimmed">
                                                        <Text span c="teal.7" fw={700}>{t('rainfall.stats.satisfactory')}</Text> {t('rainfall.stats.basinLevels')}
                                                    </Text>
                                                </>
                                            )}
                                        </Group>
                                ...
                                <Card withBorder radius="md" p="md">
                                    <Title order={4} mb="xs">{t('rainfall.registry.title')}</Title>
                                    <ScrollArea h={450}>
                                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                                <Table.Tr>
                                                    ...
                                                    <Table.Th style={{ fontSize: 11 }}>{t('rainfall.registry.rainfallHeader')}</Table.Th>
                                                </Table.Tr>
                                            </Table.Thead>
                                            <Table.Tbody>
                                                {mappedStations.length === 0 ? (
                                                    <Table.Tr>
                                                        <Table.Td colSpan={4} style={{ textAlign: 'center', padding: 16 }}>
                                                            <Text size="xs" c="dimmed">{t('rainfall.registry.empty')}</Text>
                                                        </Table.Td>
                                                    </Table.Tr>
                                                ) : (
                        ...
                        <Grid.Col span={{ base: 12, lg: 7 }}>
                            <GisMap
                                ...
                                legendTitle={t('rainfall.legend.precipitationStatus')}
                                legends={legends}
                                height={560}
                    ...
                    <Stack gap="lg">
                            ...
                            <Card withBorder radius="md" p="lg">
                                <Flex justify="space-between" align="flex-start" wrap="wrap" gap="md">
                                    ...
                                    <Stack align="flex-end" gap={2}>
                                        <Text size="xs" fw={700} c="dimmed">{t('rainfall.detail.latest24h')}</Text>
                                        <Title order={2} c="blue.8">{targetStation.value !== null ? `${targetStation.value} mm` : t('common.noData')}</Title>
                                    </Stack>
                                </Flex>
                            </Card>
                        )}
                        ...
                        <Grid gutter="lg">
                            ...
                            <Grid.Col span={{ base: 12, lg: 8 }}>
                                <Card withBorder radius="md" p="md">
                                    <Title order={4} mb="md">{t('rainfall.chart.hydrograph')}</Title>
                                    {historicalLoading ? (
                                    ...
                                    <Text size="10px" c="dimmed" mt="xs" ta="center">
                                        {t('rainfall.chart.hydrographDesc')}
                                    </Text>
                                </Card>
                            </Grid.Col>
                ...
                {/* RECENT HISTORICAL RAINFALL CRUD TABLE */}
                <Card withBorder radius="md" p="md" mt="xl">
                    <Title order={4} mb="md">{t('rainfall.history.title')}</Title>
                        ...
                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                <Table.Tr>
                                    ...
                                    <Table.Th style={{ fontSize: 11 }}>{t('rainfall.history.precipHeader')}</Table.Th>
                                ...
                                {filteredHistoryRows.length > 0 ? (
                                    ...
                                    <Table.Tr>
                                        <Table.Td colSpan={8} style={{ textAlign: 'center', padding: 16 }}>
                                            <Text size="xs" c="dimmed">{t('rainfall.empty.measurements')}</Text>
                                        </Table.Td>
                                    </Table.Tr>
                                )}
                            </Table.Tbody>
                ...
                {canManage && pendingQueue.length > 0 && (
                    <Card withBorder radius="md" p="md" mt="xl" style={{ borderLeft: '5px solid #d97706' }}>
                        <Title order={4} mb="md" c="amber.8">{t('rainfall.queue.title')} ({pendingQueue.length})</Title>
                        <Table highlightOnHover verticalSpacing="xs" withRowBorders={false} withTableBorder={false}>
                            <Table.Thead style={{ backgroundColor: 'var(--mantine-color-gray-0)' }}>
                                <Table.Tr>
                                    <Table.Th style={{ fontSize: 11 }}>{t('common.table.station')}</Table.Th>
                                    <Table.Th style={{ fontSize: 11 }}>{t('rainfall.queue.precipHeader')}</Table.Th>
                                    <Table.Th style={{ fontSize: 11 }}>{t('common.table.readingDate')}</Table.Th>
            ...
            {/* Modal: Log Rainfall */}
            <Modal opened={createOpened} onClose={closeCreate} title={<Text fw={700} size="md">{t('rainfall.modal.log')}</Text>} centered radius="md">
                <form onSubmit={handleCreateSubmit}>
                    <Stack gap="md">
                        <Select
                            label={t('common.modal.selectGaugeStation')}
                            placeholder={t('rainfall.modal.stationPlaceholder')}
                        ...
                        <NumberInput
                            label={t('rainfall.modal.valueLabel')}
                            placeholder={t('rainfall.modal.valuePlaceholder')}
            ...
            {/* Modal: Edit Rainfall */}
            <Modal opened={editOpened} onClose={closeEdit} title={<Text fw={700} size="md">{t('rainfall.modal.edit')}</Text>} centered radius="md">
                {selectedMeasurement && (
            ...
            <Modal opened={rejectOpened} onClose={closeReject} title={<Text fw={700} size="md" c="red.7">{t('rainfall.modal.reject')}</Text>} centered radius="md">
                {selectedMeasurement && (
            ...
            <Modal
                ...
                title={
                    <Group gap="xs">
                        ...
                        <Title order={5}>{t('rainfall.stats.dryAlertStations')} — {criticalStations.length}</Title>
                    </Group>
                }
                zIndex={2000}
            >
                <Table highlightOnHover verticalSpacing="xs">
                    <Table.Thead>
                        <Table.Tr>
                            ...
                            <Table.Th style={{ fontSize: 11 }}>Rainfall</Table.Th>
                        </Table.Tr>
                    </Table.Thead>
                    <Table.Tbody>
                        {criticalStations.map((s) => (
                            ...
                            >
                                ...
                                <Table.Td><Text fz="xs" c="red.7" fw={700}>{s.value?.toFixed(1)} {s.unit}</Text></Table.Td>
                            </Table.Tr>
                        ))}
                    </Table.Tbody>
                </Table>
            ...
            <MeasurementImportModal type="rainfall" opened={importOpen} onClose={closeImport} />
        </>
    );
}

Rainfall.layout = (page: React.ReactNode) => <AuthenticatedLayout>{page}</AuthenticatedLayout>;
